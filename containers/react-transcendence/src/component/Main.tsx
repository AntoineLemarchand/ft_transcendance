...
function Main(props: { component: any }) {
  ...
  const [cookie, , removeCookie] = useCookies(["auth", "userInfo", "socket"]);

  const ProcessLogout = () => {
    removeCookie("auth", { path: "/" });
    removeCookie("userInfo", { path: "/" });
    removeCookie("socket", { path: "/" });
    navigate("/");
  };
  ...
  const pages = [
    ...
    { icon: <FaCommentDots />, ref: "chat", component: <Chat /> },
    {
      icon: <FaUser />,
      ref: "profile",
      component: <Profile username={cookie["userInfo"]} />,
    },
  ];
  ...
  return (
    <main>
      <div className="links">
        {pages.map((page, idx) => (
          <Link to={"/" + page.ref} key={idx} style={tabStyle(page.ref)}>
            {page.icon}
          </Link>
        ))}
