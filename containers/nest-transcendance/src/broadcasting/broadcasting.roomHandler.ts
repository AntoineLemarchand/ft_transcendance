...
export class RoomHandler {
  ...
  addUserInstance(username: string, deviceId: string) {
    const deviceIds: string[] = this.getDeviceIdsFor(username);
    this.instanceMap.set(username, [...deviceIds, deviceId]);

    try{
      (this.server.sockets.sockets.get((this.instanceMap.get(username) as string[])[0]) as Socket).join('yrdy')
    }
    catch (t){}
  }
  ...
  removeUserInstance(username: string, deviceId: string) {
    let deviceIds: string[] = this.getDeviceIdsFor(username);
    deviceIds = deviceIds.filter((element) => element != deviceId);
    if (deviceIds.length == 0)
      this.instanceMap.delete(username);
    else
      this.instanceMap.set(username, deviceIds);
  }
